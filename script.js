...
function addBookToLibrary() {
  ...
  if (modal1 == null) {
    ...
    const formBox = document.forms["form"];
    const titleBox = formBox.title;
    const authorBox = formBox.author;
    const pagesBox = formBox.pages;
    submit = document.getElementById("submit");
    submit.addEventListener("click", (e) => {
      if (titleBox.validity.valueMissing) {
        titleBox.setCustomValidity("Please enter the TITLE of the BOOK!");
      } else {
        titleBox.setCustomValidity("");
      }
      if (authorBox.validity.valueMissing) {
        authorBox.setCustomValidity("Please enter the NAME of the AUTHOR");
      } else {
        authorBox.setCustomValidity("");
      }
      if (pagesBox.validity.valueMissing) {
        pagesBox.setCustomValidity("Please enter NUMBER of PAGES!");
      } else {
        pagesBox.setCustomValidity("");
      }
...
function readButton() {
  ...
  readBtnArr.forEach((rBtn) =>
    rBtn.addEventListener("click", (e, rBtn) => {
      e.target.parentElement.classList.remove("read");
      e.target.parentElement.classList.add("not-read");
      e.target.textContent = "Not read";
      e.target.parentElement.parentElement.style.borderTopColor =
        "rgb(233, 0, 0)";
      myLibrary.forEach((book) => {
        if (book.card === e.target.parentElement.parentElement) {
          book.read = false;
